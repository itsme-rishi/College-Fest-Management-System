...
async function createTables() {
  try {
    ...
    DROP TRIGGER IF EXISTS organiser_approved_trigger ON organiser;
    DROP FUNCTION IF EXISTS update_org_event_approved;



    CREATE OR REPLACE FUNCTION update_org_event_approved()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE org_event
...
CREATE TRIGGER organiser_approved_trigger
AFTER UPDATE OF Approved ON organiser
FOR EACH ROW
EXECUTE FUNCTION update_org_event_approved();
